...
///
/// This resource manages which phase nodes have started and finished, and maps node enums
/// to their dedicated ECS entities for targeted triggers.
#[derive(Resource)]
pub struct LoadingDag<P: LoadingDagPhase> {
    /// Mapping of phase nodes to their required dependencies.
    pub dependencies: HashMap<P, Vec<P>>,
    /// List of phase nodes that have already been triggered to start.
    pub started_nodes: Vec<P>,
    /// List of phase nodes that have successfully completed their tasks.
    pub completed_nodes: Vec<P>,
    /// Mapping of phase node enums to their dedicated ECS entities for targeted observers.
    pub node_entities: HashMap<P, Entity>,
}
...
impl<'a, P: LoadingDagPhase> LoadingPhaseProxy<'a, P> {
    ...
    pub fn add_node<M>(
        &mut self,
        node: P,
        system: impl IntoSystem<On<'static, 'static, StartNode<P>>, (), M> + Send + 'static,
    ) -> &mut Self {
        self.init_node_dag();
    ...
    pub fn add_edge(&mut self, from: P, to: P) -> &mut Self {
        self.init_node_dag();
...
pub trait LoadingAppExt {
    /// Enters a loading phase context to register nodes and edges for a specific phase enum type.
