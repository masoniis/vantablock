...
pub fn build_dense_mesh<R>(
    ...
    OPAQUE_FACE_BUFFER.with(|opaque_buffer_ref| {
        TRANSPARENT_FACE_BUFFER.with(|transparent_buffer_ref| {
            let mut opaque_faces = opaque_buffer_ref.borrow_mut();
            let mut transparent_faces = transparent_buffer_ref.borrow_mut();

            opaque_faces.clear();
            transparent_faces.clear();
                ...
                for z in 0..size {
                    for y in 0..size {
                        ...
                        for &face_side in &FaceSide::ALL {
                            ...
                            let neighbor_id = padded_chunk.get_block(
                                neighbor_pos.x,
                                neighbor_pos.y,
                                neighbor_pos.z,
                            );
